...
#define FUNCTION_BLOCK_MAPS 2
//#define FUNCTION_SOUND

#define SELECT_MAPS 4
#define PRE_START_TIME 2
#define VOTE_TIME 8
...
new const PREFIX[] = "^x04[Menedzer map]^x01";
...
public plugin_natives()
{
	...
	register_native("showMapVoteMenu", "native_showMapVoteMenu");
}

public native_showMapVoteMenu(pluginIndex, params)
{
	StartVote(0);
}
...
public Command_BlockedCmds(id)
{
	if(g_bNightMode)
	{
		console_print(id, "Komenda niedostepna w trakcie night mode'a.");
...
public Command_StartVote(id, flag)
{
	...
	{
		SetNewRoundVote();
		ColorChat(0, NORMAL, "%s Glosowanie na nastepna mape rozpocznie sie w nastepnej rundzie.", PREFIX);
	}
...
public Command_StopVote(id, flag)
{
	...
	{		
		...
		ColorChat(0, NORMAL, "%s^x04 %s^x01 wylaczyl glosowanie na mape.", PREFIX, szName);
	}
...
public Command_Timeleft(id)
{
	...
	{
		new szText[128], len;
		len = formatex(szText, charsmax(szText), "Czas do konca mapy ");
		if(iWinLimit)
		{
			new iLeftWins = iWinLimit - max(g_iTeamScore[0], g_iTeamScore[1]);
			new szWins[16]; get_ending(iLeftWins, "wygranych", "wygranych", "wygranych", szWins, charsmax(szWins));
			len += formatex(szText[len], charsmax(szText) - len, "%d %L", iLeftWins, LANG_PLAYER, szWins);
		}
		if(iWinLimit && iMaxRounds)
		{
			len += formatex(szText[len], charsmax(szText) - len, " lub ");
		}
		if(iMaxRounds)
		{
			new iLeftRounds = iMaxRounds - g_iTeamScore[0] - g_iTeamScore[1];
			new szRounds[16]; get_ending(iLeftRounds, "rund", "rund", "rund", szRounds, charsmax(szRounds));
			len += formatex(szText[len], charsmax(szText) - len, "%d %L", iLeftRounds, LANG_PLAYER, szRounds);
		}
		ColorChat(0, NORMAL, "%s %s.", PREFIX, szText);
	}
	else
	{
		...
		{
			new a = get_timeleft();
			ColorChat(0, NORMAL, "%s Czas do konca mapy: %d:%02d", PREFIX, (a / 60), (a % 60));
		}
		else
		{
			ColorChat(0, NORMAL, "%s Mapa nie ma limitu czasowego.", PREFIX);
		}
	}
}
...
public Command_TheTime(id)
{
	...
	ColorChat(0, NORMAL, "%s Aktualna godzina:^x03 %s^x01.", PREFIX, szTime);
}
...
{
	new szMap[32];
	get_pcvar_string(g_pCvars[NEXTMAP], szMap, charsmax(szMap));
	ColorChat(0, NORMAL, "%s Nastepna mapa: %s.", PREFIX, szMap);
}

public Command_CurrentMap(id)
{
	ColorChat(0, NORMAL, "%s Aktualna mapa:^x04 %s^x01.", PREFIX, g_szCurrentMap);
}
#endif
...
public Command_RockTheVote(id)
{
	...
	if(g_bNightMode && g_bNightModeOneMap)
	{
		ColorChat(id, NORMAL, "%s Komenda niedostepna w trakcie night mode'a", PREFIX);
		return PLUGIN_HANDLED;
	}
	#endif
	...
	if(iTime > 0)
	{
		ColorChat(id, NORMAL, "%s Komenda RTV jest jeszcze niedostepna. Poczekaj:^x03 %d^x01:^x03%02d^x01.", PREFIX, iTime / 60, iTime % 60);
		return PLUGIN_HANDLED;
	}
	...
	if(iVotes <= 0)
	{
		...
		{
			StartVote(0);
			ColorChat(0, NORMAL, "%s Rozpoczynanie wczesnego glosowania na mape.", PREFIX);
		}
		else
		{
			SetNewRoundVote();
			ColorChat(0, NORMAL, "%s Glosowanie rozpocznie sie w nastepnej rundzie.", PREFIX);
		}
		return PLUGIN_HANDLED;
	}
	
	new szVote[16];	get_ending(iVotes, "glosow", "glosow", "glosow", szVote, charsmax(szVote));
	...
	if(!g_bRockVoted[id])
	{
		...
		ColorChat(0, NORMAL, "%s^x03 %s^x01 zaglosowal na zmiane mapy. Potrzeba jeszcze:^x03 %d^x01.", PREFIX, szName, iVotes);
	}
	else
	{
		ColorChat(id, NORMAL, "%s Glosowales juz na zmiane mapy. Pozostalo:^x03 %i glosow^x01.", PREFIX, iVotes);
	}
...
{
	new szText[64]; formatex(szText, charsmax(szText), "Nominowane mapy:");
...
NominateMap(id, map[32], map_index)
{
	...
	if(eMapInfo[m_BlockCount])
	{
		ColorChat(id, NORMAL, "%s Ta mapa nie jest dostepna do nominacji.", PREFIX);
		return 0;
	}
	#endif
	...
	if(nominate_index)
	{
		...
		if(id == eNomInfo[n_Player])
		{
			...
			if(g_iLastDenominate[id] + MIN_DENOMINATE_TIME <= iSysTime)
			{
				...
				ColorChat(0, NORMAL, "%s^x04 %s^x01 wykluczyl z nominacji mape^x04 %s^x01.", PREFIX, szName, map);
				return 2;
			}
			ColorChat(id, NORMAL, "%s Nie spamuj wykluczeniem z nominacji.", PREFIX);
			return 0;
		}
		ColorChat(id, NORMAL, "%s Ta mapa jest juz nominowana.", PREFIX);
		return 0;
	}
	
	if(g_iNominatedMaps[id] >= NOMINATED_MAPS_PER_PLAYER)
	{
		ColorChat(id, NORMAL, "%s Nie mozesz juz nominowac map.", PREFIX);
		return 0;
	}
	...
	if(get_pcvar_num(g_pCvars[NOMINATION_DEL_NON_CUR_ONLINE]))
	{
		...
		ColorChat(0, NORMAL, "%s^x04 %s^x01 nominowal^x04 %s^x01 dla %d-%d graczy.", PREFIX, szName, map, iMinPlayers, eMapInfo[m_MaxPlayers]);
	}
	else
	{
		ColorChat(0, NORMAL, "%s^x04 %s^x01 nominowal^x04 %s^x01.", PREFIX, szName, map);
	}
...
Show_MapsListMenu(id)
{
	new szText[64]; formatex(szText, charsmax(szText), "Lista map:");
...
#endif
public changeMap()
	set_task(get_cvar_float("mp_chattime"), "delayedChangeMap");

public delayedChangeMap()
{
	new szMap[32], command[50];
	...
	formatex(command, charsmax(command), "changelevel ^"%s^"", szMap);
	server_cmd(command);
}
...
public Event_NewRound()
{
	...
	if(g_bNightMode && g_bVoteFinished && (g_bNightModeOneMap && get_pcvar_num(g_pCvars[CHANGE_TYPE]) >= 1 || !g_bCurMapInNightMode))
	{
		...
		ColorChat(0, NORMAL, "%s Night mode. Nastepna mapa:^x04 %s^x01.", PREFIX, szMapName);
		return;
	}
	#endif
	...
	#endif
	{
		Intermission();
		new szMapName[32]; get_pcvar_string(g_pCvars[NEXTMAP], szMapName, charsmax(szMapName));
		ColorChat(0, NORMAL, "%s Nastepna mapa:^x03 %s^x01.", PREFIX, szMapName);
	}
}
...
public Task_CheckNight()
{
	...
	if(g_bNightMode && !bOldNightMode)// NightMode ON
	{
		if(g_bNightModeOneMap)
		{
			if(g_bCurMapInNightMode)
			{
				...
				ColorChat(0, NORMAL, "%s Night mode dziala od godziny:^x03 %02d^x01:^x03%02d^x01.", PREFIX, iEndHour, iEndMinutes);
			}
			else
			{
				...
				{
					Intermission();
					ColorChat(0, NORMAL, "%s Zmiana mapy na nocna:^x04 %s^x01.", PREFIX, szMapName);
				}
				else
				{
					g_bVoteFinished = true;
					ColorChat(0, NORMAL, "%s Zmiana mapy na nocna w nastepnej rundzie:^x04 %s^x01.", PREFIX szMapName);
				}
			}
		}
		else if(!g_bCurMapInNightMode)
		{
			if(get_pcvar_num(g_pCvars[START_VOTE_IN_NEW_ROUND]) == 0)
			{
				ColorChat(0, NORMAL, "%s Zmiana mapy na^x04 nocne^x01.", PREFIX);
				StartVote(0);
			}
			else
			{
				SetNewRoundVote();
				ColorChat(0, NORMAL, "%s Zmiana mapy na nocna w nastepnej rundzie.", PREFIX);
			}
		}
	}
	else if(!g_bNightMode && bOldNightMode)// NightMode OFF
	{
		...
		ColorChat(0, NORMAL, "%s Night mode zostal wylaczony.", PREFIX);
	}
}
#endif
...
public VoteMenu(id)
{
	...
	iLen = formatex(szMenu[iLen], charsmax(szMenu) - iLen, "\y%s:^n^n", g_bPlayerVoted[id] ? "Wyniki glosowania" : "Wybierz mape");
	...
	new szSec[16]; get_ending(g_iTimer, "sekund", "sekund", "sekund", szSec, charsmax(szSec));
	iLen += formatex(szMenu[iLen], charsmax(szMenu) - iLen, "^n\dPozostalo \r%d\d sekund", g_iTimer);
...
public VoteMenu_Handler(id, key)
{
	...
	if(get_pcvar_num(g_pCvars[SHOW_SELECTS]))
	{
		...
		{
			ColorChat(0, NORMAL, "%s^x04 %s^x01 wybral przedluzenie mapy.", szName);
		}
		else
		{
			ColorChat(0, NORMAL, "%s^x04 %s^x01 wybral mape:^x03 %s^x01.", PREFIX, szName, g_eMenuItems[key][v_MapName]);
		}
	}
...
FinishVote()
{
	...
	if(!g_iTotalVotes || (iMaxVote != g_iMenuItemsCount))
	{
		if(g_iTotalVotes)
		{
			ColorChat(0, NORMAL, "%s Nastepna mapa:^x03 %s^x01.", PREFIX, g_eMenuItems[iMaxVote][v_MapName]);
		}
		else
		{
			iMaxVote = random_num(0, g_iMenuItemsCount - 1);
			ColorChat(0, NORMAL, "%s Nikt nie glosowal. Nastepna mapa bedzie:^x04 %s^x01.", PREFIX, g_eMenuItems[iMaxVote][v_MapName]);
		}
		...
		if(get_pcvar_num(g_pCvars[LAST_ROUND]))
		{
			g_fOldTimeLimit = get_pcvar_float(g_pCvars[TIMELIMIT]);
			set_pcvar_float(g_pCvars[TIMELIMIT], 0.0);
			ColorChat(0, NORMAL, "%s Ostatnia runda.", PREFIX);
		}
		#if defined FUNCTION_RTV
		...
		#endif
		{
			...
			new szSec[16]; get_ending(iSec, "sekund", "sekund", "sekund", szSec, charsmax(szSec));
			ColorChat(0, NORMAL, "%s Mapa zostanie zmieniona za^x03 %d^x01 sekund.", PREFIX, iSec);
			Intermission();
		}
		#if defined FUNCTION_RTV
		...
		#endif
		{
			ColorChat(0, NORMAL, "%s Zmiana mapy nastapi w nastepnej rundzie.", PREFIX);
		}
	}
	else
	{
		...
		if(get_pcvar_num(g_pCvars[EXTENDED_TYPE]) == 1 && (iWinLimit || iMaxRounds))
		{
			...
			new szRounds[16]; get_ending(iRounds, "rund", "rund", "rund", szRounds, charsmax(szRounds));
			ColorChat(0, NORMAL, "%s Aktualna mapa zostala przedluzona o^x03 %d^x01 rund.", PREFIX, iRounds);
		}
		else
		{
			new iMin = get_pcvar_num(g_pCvars[EXTENDED_TIME]);
			new szMin[16]; get_ending(iMin, "minut", "minut", "minut", szMin, charsmax(szMin));
			
			ColorChat(0, NORMAL, "%s Aktualna mapa zostala przedluzona o^x03 %d^x01 minut.", PREFIX, iMin);
